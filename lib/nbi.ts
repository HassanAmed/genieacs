...
This  file implements nbi module which exposes a REST API on configured ip:port.
This one is only required if you have an external system integrating with GenieACS using this API.

From version 1.2.0 genieacs-gui is deprecated see pre-release notes
(https://github.com/genieacs/genieacs/releases).Now the platform is independent of genieacs-gui and
a northern interface server but nbi can be used if we want to add a new custom UI to the platform.
