...
This  file creates collections (db schema) and implements db functions which are used by cwmp-server. Such as saveDevice to store device
in db upon discovery

Both db files in lib and ui connect to same db the functions in both are different and specific to 
